...
module.exports = {
  name: 'derank',
  // Usage: &derank @membre @role  (ou ID/nom de role)
  async execute(message, args) {
    ...
    if (!role) {
      return message.reply({ embeds: [errorEmbed('Mentionne, donne l\'ID ou le nom exact du role a retirer.')] });
    }

    if (role.position >= message.guild.members.me.roles.highest.position) {
      return message.reply({ embeds: [errorEmbed('Ce role est trop haut pour que je puisse le retirer.')] });
    }

    await target.roles.remove(role);
    return message.reply({ embeds: [successEmbed(`Le role **${role.name}** a ete retire a **${target.user.tag}**.`)] });
  },
};
